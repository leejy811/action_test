...
public class Player : MonoBehaviour
{
    ...
    void Reload()
    {
        ...
        if(rDown && !isDodge && !isSwap && !IsJump() && isFireReady && !isShop)
        {
    ...
    void Dodge()
    {
        if (jDown && !IsJump() && !isDodge && moveVec != Vector3.zero && !isSwap && !isShop)
        {
            dodgeVec = moveVec;
            speed *= 2;
    ...
    void Swap()
    {
        ...
        if ((sDown[0] || sDown[1] || sDown[2]) && !IsJump() && !isDodge && !isShop)
        {
            if (equipWeapon != null)
